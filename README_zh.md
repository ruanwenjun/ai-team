...
| `--lang {zh|en}` | 当前这次 `init-team` 或 `run-team` 的语言；如果省略，skill 每次都会询问，并根据本次输入语言推荐中文或英文 | 每次询问 |
...
在开始生成前，`init-team` 会先确定语言偏好。如果没有传 `--lang`，它每次都会询问，并根据本次输入语言推荐中文或英文。
...
在进入第一个阶段前，`run-team` 也会先确定语言偏好。如果没有传 `--lang`，它每次都会询问，并根据本次输入语言推荐中文或英文。对于 `--issue`，本次选择的语言只影响这次新追加的内容，不会重写历史记录。
