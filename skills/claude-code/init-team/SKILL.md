...
4. **Create directories** using Bash tool with `mkdir -p`:
   ```
   ...
   ```
5. **Generate files** using the Write tool for each file. Process roles in order, applying naming rules (single role = no number, multiple = numbered). Generate all files:
   - `.ai-team/team.md`
