...
5. **Create directories** using shell commands with `mkdir -p`:
   ...
   ```
6. **Generate files** using the platform's file-writing mechanism for each file. Process roles in order, applying naming rules (single role = no number, multiple = numbered). Generate all files:
   - `.ai-team/team.md`
