...
| All roles — parallel subagents | Claim all remaining `pending` roles in the current stage, then dispatch them as subagents concurrently. Enter the stage gate only after every role assigned to the stage is `done`. |
